...
def update():
    ...
    while b['pagination']['page']!=b['pagination']['pages']:
        for k in b["users"]:
            ...
            prev_users[k["id"]] =  {"display_name": k["display_name"],"votes_count": k["votes_count"],"avatar": img}
            print('updated list')
...
async def main():
    ...
    sigma = threading.Thread(target=update)
    sigma.start()
    ...
    while True:
        ...
        k = 0
        print(prev_users)
        for i in prev_users.copy():
            pygame.draw.rect(screen,(0,0,0),(base_rect[0],base_rect[1]+(k*90),base_rect[2],base_rect[3]),3,20)
            screen.blit(prev_users[i]["avatar"],(base_rect[0]+20,base_rect[1]+10+(k*90)))
            k+=1
            print('used')
            if k>10:
                break
